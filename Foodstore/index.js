...
if(recipeData.length !== 0) {
    document.querySelector("#recipe-header").textContent = "Here are your Recipes";
    ...
    recipeData.forEach((element, currentIndex = -1) => {
      ...
      container2.appendChild(recipeTitle);
        ...
        const cardButton = `<hr><span class = "info"><span style = "color: red">${Math.round(element.recipe.calories)} CALORIES</span><hr><span  style = "color: green">${element.recipe.ingredients.length} INGREDIENTS</span></span><hr><span class="author">${element.recipe.source}</span>`;
      container2.insertAdjacentHTML("beforeend", cardButton);

      container.appendChild(container2);
      recipeContainer.appendChild(container);
        container.onclick = () => {seeDetails(currentIndex)};
...
function seeDetails(index) {
  ...
  document.querySelector("#ingredients").textContent ="";
   ...
   recipeNutrition.textContent = recipeData[index].recipe.healthLabels.join(", ");
   let instructions = document.querySelector("#instructions");
   instructions.onclick = () => window.open( recipeData[index].recipe.url, "_blank");
